...
for scope in --user --system; do
  if flatpak info "$scope" app.grayjay.Grayjay &>/dev/null; then
    flatpak update "$scope" app.grayjay.Grayjay -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "app.grayjay.Grayjay is not installed in any flatpak installation." >&2
  exit 1
fi
