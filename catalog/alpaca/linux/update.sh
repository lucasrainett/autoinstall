...
for scope in --user --system; do
  if flatpak info "$scope" com.jeffser.Alpaca &>/dev/null; then
    flatpak update "$scope" com.jeffser.Alpaca -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.jeffser.Alpaca is not installed in any flatpak installation." >&2
  exit 1
fi
