...
for scope in --user --system; do
  if flatpak info "$scope" com.obsproject.Studio &>/dev/null; then
    flatpak update "$scope" com.obsproject.Studio -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.obsproject.Studio is not installed in any flatpak installation." >&2
  exit 1
fi
