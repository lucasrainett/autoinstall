...
if [ -d "/Applications/Pages.app" ]; then
  echo "Pages is already installed."
  exit 0
fi
...
echo "Pages is distributed through the Mac App Store, which cannot be driven unattended."
echo "Opening its App Store page — press Get/Download there to reinstall it."
open "macappstore://apps.apple.com/app/id409201541" 2>/dev/null || true
