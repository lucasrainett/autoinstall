...
for scope in --user --system; do
  if flatpak info "$scope" io.github.ungoogled_software.ungoogled_chromium &>/dev/null; then
    flatpak update "$scope" io.github.ungoogled_software.ungoogled_chromium -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "io.github.ungoogled_software.ungoogled_chromium is not installed in any flatpak installation." >&2
  exit 1
fi
