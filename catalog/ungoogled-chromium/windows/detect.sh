...
winget list --id eloston.ungoogled-chromium -e --accept-source-agreements 2>/dev/null | grep -qi "eloston.ungoogled-chromium" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id eloston.ungoogled-chromium -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "eloston.ungoogled-chromium"; then
  exit 2
fi
exit 0
