...
flatpak list --columns=application 2>/dev/null | grep -qx "com.rustdesk.RustDesk" || exit 1
...
scope=--user
flatpak info --user com.rustdesk.RustDesk &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.rustdesk.RustDesk 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.rustdesk.RustDesk --show-commit 2>/dev/null)
