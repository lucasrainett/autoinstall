...
flatpak list --columns=application 2>/dev/null | grep -qx "com.mattjakeman.ExtensionManager" || exit 1
...
scope=--user
flatpak info --user com.mattjakeman.ExtensionManager &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.mattjakeman.ExtensionManager 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.mattjakeman.ExtensionManager --show-commit 2>/dev/null)
