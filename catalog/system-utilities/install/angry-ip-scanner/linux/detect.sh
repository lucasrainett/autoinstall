...
flatpak list --columns=application 2>/dev/null | grep -qx "org.angryip.ipscan" || exit 1
...
scope=--user
flatpak info --user org.angryip.ipscan &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit org.angryip.ipscan 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub org.angryip.ipscan --show-commit 2>/dev/null)
