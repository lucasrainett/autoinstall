...
flatpak list --columns=application 2>/dev/null | grep -qx "io.github.ilya_zlobintsev.LACT" || exit 1
...
scope=--user
flatpak info --user io.github.ilya_zlobintsev.LACT &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit io.github.ilya_zlobintsev.LACT 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub io.github.ilya_zlobintsev.LACT --show-commit 2>/dev/null)
