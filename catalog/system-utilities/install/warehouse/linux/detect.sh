...
flatpak list --columns=application 2>/dev/null | grep -qx "io.github.flattool.Warehouse" || exit 1
...
scope=--user
flatpak info --user io.github.flattool.Warehouse &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit io.github.flattool.Warehouse 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub io.github.flattool.Warehouse --show-commit 2>/dev/null)
