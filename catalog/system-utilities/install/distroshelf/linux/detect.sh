...
flatpak list --columns=application 2>/dev/null | grep -qx "com.ranfdev.DistroShelf" || exit 1
...
scope=--user
flatpak info --user com.ranfdev.DistroShelf &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.ranfdev.DistroShelf 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.ranfdev.DistroShelf --show-commit 2>/dev/null)
