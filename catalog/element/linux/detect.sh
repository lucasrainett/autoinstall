...
flatpak list --columns=application 2>/dev/null | grep -qx "im.riot.Riot" || exit 1
...
scope=--user
flatpak info --user im.riot.Riot &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit im.riot.Riot 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub im.riot.Riot --show-commit 2>/dev/null)
