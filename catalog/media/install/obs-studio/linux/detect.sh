...
flatpak list --columns=application 2>/dev/null | grep -qx "com.obsproject.Studio" || exit 1
...
scope=--user
flatpak info --user com.obsproject.Studio &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.obsproject.Studio 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.obsproject.Studio --show-commit 2>/dev/null)
