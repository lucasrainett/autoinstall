...
flatpak list --columns=application 2>/dev/null | grep -qx "app.organicmaps.desktop" || exit 1
...
scope=--user
flatpak info --user app.organicmaps.desktop &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit app.organicmaps.desktop 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub app.organicmaps.desktop --show-commit 2>/dev/null)
