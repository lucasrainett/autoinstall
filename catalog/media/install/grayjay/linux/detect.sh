...
flatpak list --columns=application 2>/dev/null | grep -qx "app.grayjay.Grayjay" || exit 1
...
installed=$(flatpak info "$scope" --show-commit app.grayjay.Grayjay 2>/dev/null) || exit 0
