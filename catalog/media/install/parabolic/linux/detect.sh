...
flatpak list --columns=application 2>/dev/null | grep -qx "org.nickvision.tubeconverter" || exit 1
...
scope=--user
flatpak info --user org.nickvision.tubeconverter &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit org.nickvision.tubeconverter 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub org.nickvision.tubeconverter --show-commit 2>/dev/null)
