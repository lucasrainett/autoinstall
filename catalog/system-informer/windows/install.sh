...
if winget list --id WinsiderSS.SystemInformer -e --accept-source-agreements 2>/dev/null | grep -qi "WinsiderSS.SystemInformer"; then
  echo "WinsiderSS.SystemInformer is already installed; nothing to do."
  exit 0
fi
