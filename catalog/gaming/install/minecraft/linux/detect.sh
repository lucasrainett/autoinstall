...
flatpak list --columns=application 2>/dev/null | grep -qx "com.mojang.Minecraft" || exit 1
...
scope=--user
flatpak info --user com.mojang.Minecraft &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.mojang.Minecraft 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.mojang.Minecraft --show-commit 2>/dev/null)
