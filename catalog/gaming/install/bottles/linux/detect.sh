...
flatpak list --columns=application 2>/dev/null | grep -qx "com.usebottles.bottles" || exit 1
...
scope=--user
flatpak info --user com.usebottles.bottles &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.usebottles.bottles 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.usebottles.bottles --show-commit 2>/dev/null)
