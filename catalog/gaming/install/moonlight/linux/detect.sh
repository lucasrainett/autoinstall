...
flatpak list --columns=application 2>/dev/null | grep -qx "com.moonlight_stream.Moonlight" || exit 1
...
scope=--user
flatpak info --user com.moonlight_stream.Moonlight &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.moonlight_stream.Moonlight 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.moonlight_stream.Moonlight --show-commit 2>/dev/null)
