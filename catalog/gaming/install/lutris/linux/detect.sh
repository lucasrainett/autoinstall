...
flatpak list --columns=application 2>/dev/null | grep -qx "net.lutris.Lutris" || exit 1
...
scope=--user
flatpak info --user net.lutris.Lutris &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit net.lutris.Lutris 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub net.lutris.Lutris --show-commit 2>/dev/null)
