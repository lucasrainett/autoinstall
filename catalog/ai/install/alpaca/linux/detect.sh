...
flatpak list --columns=application 2>/dev/null | grep -qx "com.jeffser.Alpaca" || exit 1
...
scope=--user
flatpak info --user com.jeffser.Alpaca &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.jeffser.Alpaca 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.jeffser.Alpaca --show-commit 2>/dev/null)
