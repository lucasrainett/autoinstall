...
winget list --id Volta.Volta -e --accept-source-agreements 2>/dev/null | grep -qi "Volta.Volta" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Volta.Volta -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Volta.Volta"; then
  exit 2
fi
exit 0
