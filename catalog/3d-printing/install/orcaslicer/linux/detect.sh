...
flatpak list --columns=application 2>/dev/null | grep -qx "com.orcaslicer.OrcaSlicer" || exit 1
...
scope=--user
flatpak info --user com.orcaslicer.OrcaSlicer &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.orcaslicer.OrcaSlicer 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.orcaslicer.OrcaSlicer --show-commit 2>/dev/null)
