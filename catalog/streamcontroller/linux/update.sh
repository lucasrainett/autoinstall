...
for scope in --user --system; do
  if flatpak info "$scope" com.core447.StreamController &>/dev/null; then
    flatpak update "$scope" com.core447.StreamController -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.core447.StreamController is not installed in any flatpak installation." >&2
  exit 1
fi
