...
# WinBoat ships AppImage/deb/rpm builds on GitHub and no repository of its own. Gear Lever does the
# desktop integration (launcher entry, icon, update tracking) instead of leaving a loose executable
# in a folder with no way to launch it.
...
VERSION=$(curl -fsSL https://api.github.com/repos/TibixDev/winboat/releases/latest |
  grep -oP '"tag_name": "\K[^"]+')
# The tag carries a leading "v" that the asset filename does not.
BARE=${VERSION#v}
# The filename here becomes the name Gear Lever stores it under, so keep it stable and lowercase.
curl -fsSL -o "$TMP_DIR/winboat.appimage" \
  "https://github.com/TibixDev/winboat/releases/download/${VERSION}/winboat-${BARE}-x86_64.AppImage"
chmod +x "$TMP_DIR/winboat.appimage"

flatpak run it.mijorus.gearlever --integrate "$TMP_DIR/winboat.appimage" -y
