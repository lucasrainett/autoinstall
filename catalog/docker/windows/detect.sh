...
winget list --id Docker.DockerDesktop -e --accept-source-agreements 2>/dev/null | grep -qi "Docker.DockerDesktop" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Docker.DockerDesktop -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Docker.DockerDesktop"; then
  exit 2
fi
exit 0
