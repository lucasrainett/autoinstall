...
flatpak list --columns=application 2>/dev/null | grep -qx "fr.handbrake.ghb" || exit 1
...
scope=--user
flatpak info --user fr.handbrake.ghb &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit fr.handbrake.ghb 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub fr.handbrake.ghb --show-commit 2>/dev/null)
