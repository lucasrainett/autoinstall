...
winget list --id Hashicorp.Terraform -e --accept-source-agreements 2>/dev/null | grep -qi "Hashicorp.Terraform" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Hashicorp.Terraform -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Hashicorp.Terraform"; then
  exit 2
fi
exit 0
