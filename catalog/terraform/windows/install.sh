...
if winget list --id Hashicorp.Terraform -e --accept-source-agreements 2>/dev/null | grep -qi "Hashicorp.Terraform"; then
  echo "Hashicorp.Terraform is already installed; nothing to do."
  exit 0
fi
