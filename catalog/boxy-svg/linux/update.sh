...
for scope in --user --system; do
  if flatpak info "$scope" com.boxy_svg.BoxySVG &>/dev/null; then
    flatpak update "$scope" com.boxy_svg.BoxySVG -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.boxy_svg.BoxySVG is not installed in any flatpak installation." >&2
  exit 1
fi
