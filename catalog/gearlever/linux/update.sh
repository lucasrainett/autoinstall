...
for scope in --user --system; do
  if flatpak info "$scope" it.mijorus.gearlever &>/dev/null; then
    flatpak update "$scope" it.mijorus.gearlever -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "it.mijorus.gearlever is not installed in any flatpak installation." >&2
  exit 1
fi
