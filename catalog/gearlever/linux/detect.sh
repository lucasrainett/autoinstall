...
scope=--user
flatpak info --user it.mijorus.gearlever &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit it.mijorus.gearlever 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub it.mijorus.gearlever --show-commit 2>/dev/null)
