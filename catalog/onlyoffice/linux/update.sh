...
for scope in --user --system; do
  if flatpak info "$scope" org.onlyoffice.desktopeditors &>/dev/null; then
    flatpak update "$scope" org.onlyoffice.desktopeditors -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.onlyoffice.desktopeditors is not installed in any flatpak installation." >&2
  exit 1
fi
