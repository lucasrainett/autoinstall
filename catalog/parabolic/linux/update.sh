...
for scope in --user --system; do
  if flatpak info "$scope" org.nickvision.tubeconverter &>/dev/null; then
    flatpak update "$scope" org.nickvision.tubeconverter -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.nickvision.tubeconverter is not installed in any flatpak installation." >&2
  exit 1
fi
