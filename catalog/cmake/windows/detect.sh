...
winget list --id Kitware.CMake -e --accept-source-agreements 2>/dev/null | grep -qi "Kitware.CMake" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Kitware.CMake -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Kitware.CMake"; then
  exit 2
fi
exit 0
