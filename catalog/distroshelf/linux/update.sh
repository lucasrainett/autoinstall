...
for scope in --user --system; do
  if flatpak info "$scope" com.ranfdev.DistroShelf &>/dev/null; then
    flatpak update "$scope" com.ranfdev.DistroShelf -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.ranfdev.DistroShelf is not installed in any flatpak installation." >&2
  exit 1
fi
