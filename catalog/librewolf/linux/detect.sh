...
flatpak list --columns=application 2>/dev/null | grep -qx "io.gitlab.librewolf-community" || exit 1
...
scope=--user
flatpak info --user io.gitlab.librewolf-community &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit io.gitlab.librewolf-community 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub io.gitlab.librewolf-community --show-commit 2>/dev/null)
