...
winget list --id LibreWolf.LibreWolf -e --accept-source-agreements 2>/dev/null | grep -qi "LibreWolf.LibreWolf" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id LibreWolf.LibreWolf -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "LibreWolf.LibreWolf"; then
  exit 2
fi
exit 0
