...
for scope in --user --system; do
  if flatpak info "$scope" com.vscodium.codium &>/dev/null; then
    flatpak update "$scope" com.vscodium.codium -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.vscodium.codium is not installed in any flatpak installation." >&2
  exit 1
fi
