...
flatpak list --columns=application 2>/dev/null | grep -qx "com.vscodium.codium" || exit 1
...
scope=--user
flatpak info --user com.vscodium.codium &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.vscodium.codium 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.vscodium.codium --show-commit 2>/dev/null)
