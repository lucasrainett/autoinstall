...
for scope in --user --system; do
  if flatpak info "$scope" ai.lmstudio.lm-studio &>/dev/null; then
    flatpak update "$scope" ai.lmstudio.lm-studio -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "ai.lmstudio.lm-studio is not installed in any flatpak installation." >&2
  exit 1
fi
