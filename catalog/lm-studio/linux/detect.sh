...
flatpak list --columns=application 2>/dev/null | grep -qx "ai.lmstudio.lm-studio" || exit 1
...
scope=--user
flatpak info --user ai.lmstudio.lm-studio &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit ai.lmstudio.lm-studio 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub ai.lmstudio.lm-studio --show-commit 2>/dev/null)
