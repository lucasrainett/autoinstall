...
flatpak list --columns=application 2>/dev/null | grep -qx "com.github.marktext.marktext" || exit 1
...
scope=--user
flatpak info --user com.github.marktext.marktext &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.github.marktext.marktext 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.github.marktext.marktext --show-commit 2>/dev/null)
