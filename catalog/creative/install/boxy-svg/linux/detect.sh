...
flatpak list --columns=application 2>/dev/null | grep -qx "com.boxy_svg.BoxySVG" || exit 1
...
scope=--user
flatpak info --user com.boxy_svg.BoxySVG &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.boxy_svg.BoxySVG 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.boxy_svg.BoxySVG --show-commit 2>/dev/null)
