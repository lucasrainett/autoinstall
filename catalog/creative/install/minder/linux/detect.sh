...
flatpak list --columns=application 2>/dev/null | grep -qx "com.github.phase1geo.minder" || exit 1
...
scope=--user
flatpak info --user com.github.phase1geo.minder &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.github.phase1geo.minder 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.github.phase1geo.minder --show-commit 2>/dev/null)
