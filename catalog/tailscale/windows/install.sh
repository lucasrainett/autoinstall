...
if winget list --id Tailscale.Tailscale -e --accept-source-agreements 2>/dev/null | grep -qi "Tailscale.Tailscale"; then
  echo "Tailscale.Tailscale is already installed; nothing to do."
  exit 0
fi
