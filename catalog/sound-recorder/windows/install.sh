...
echo "Sound Recorder is distributed through the Microsoft Store, which cannot be driven unattended."
echo "Opening the Store — search results for it will appear there."
powershell.exe -NoProfile -Command "Start-Process 'ms-windows-store://search/?query=Sound Recorder'" 2>/dev/null || true
