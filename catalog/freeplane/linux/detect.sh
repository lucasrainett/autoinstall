...
flatpak list --columns=application 2>/dev/null | grep -qx "org.freeplane.App" || exit 1
...
scope=--user
flatpak info --user org.freeplane.App &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit org.freeplane.App 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub org.freeplane.App --show-commit 2>/dev/null)
