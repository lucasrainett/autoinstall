...
for scope in --user --system; do
  if flatpak info "$scope" info.beyondallreason.bar &>/dev/null; then
    flatpak update "$scope" info.beyondallreason.bar -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "info.beyondallreason.bar is not installed in any flatpak installation." >&2
  exit 1
fi
