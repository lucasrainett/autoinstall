...
flatpak list --columns=application 2>/dev/null | grep -qx "info.beyondallreason.bar" || exit 1
...
scope=--user
flatpak info --user info.beyondallreason.bar &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit info.beyondallreason.bar 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub info.beyondallreason.bar --show-commit 2>/dev/null)
