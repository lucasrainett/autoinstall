...
for scope in --user --system; do
  if flatpak info "$scope" com.mattjakeman.ExtensionManager &>/dev/null; then
    flatpak update "$scope" com.mattjakeman.ExtensionManager -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.mattjakeman.ExtensionManager is not installed in any flatpak installation." >&2
  exit 1
fi
