...
winget list --id zumoshi.BrowserSelect -e --accept-source-agreements 2>/dev/null | grep -qi "zumoshi.BrowserSelect" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id zumoshi.BrowserSelect -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "zumoshi.BrowserSelect"; then
  exit 2
fi
exit 0
