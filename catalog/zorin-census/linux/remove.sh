...
PACKAGES=(zorin-os-census)

# `apt remove` silently drags out every reverse-dependency too. Measured on a real machine:
# removing `curl` would also have uninstalled Steam. Uninstalling one catalog entry must never
...
# The identifier is stored separately from the package and survives a plain removal, so a later
# reinstall would resume reporting under the same ID.
sudo rm -f /var/lib/zorin-os-census/uuid
