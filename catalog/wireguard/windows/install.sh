...
if winget list --id WireGuard.WireGuard -e --accept-source-agreements 2>/dev/null | grep -qi "WireGuard.WireGuard"; then
  echo "WireGuard.WireGuard is already installed; nothing to do."
  exit 0
fi
