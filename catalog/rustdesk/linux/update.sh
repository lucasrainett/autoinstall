...
for scope in --user --system; do
  if flatpak info "$scope" com.rustdesk.RustDesk &>/dev/null; then
    flatpak update "$scope" com.rustdesk.RustDesk -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.rustdesk.RustDesk is not installed in any flatpak installation." >&2
  exit 1
fi
