...
flatpak list --columns=application 2>/dev/null | grep -qx "io.missioncenter.MissionCenter" || exit 1
...
scope=--user
flatpak info --user io.missioncenter.MissionCenter &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit io.missioncenter.MissionCenter 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub io.missioncenter.MissionCenter --show-commit 2>/dev/null)
