...
for scope in --user --system; do
  if flatpak info "$scope" io.missioncenter.MissionCenter &>/dev/null; then
    flatpak update "$scope" io.missioncenter.MissionCenter -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "io.missioncenter.MissionCenter is not installed in any flatpak installation." >&2
  exit 1
fi
