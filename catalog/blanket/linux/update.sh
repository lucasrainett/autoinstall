...
for scope in --user --system; do
  if flatpak info "$scope" com.rafaelmardojai.Blanket &>/dev/null; then
    flatpak update "$scope" com.rafaelmardojai.Blanket -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.rafaelmardojai.Blanket is not installed in any flatpak installation." >&2
  exit 1
fi
