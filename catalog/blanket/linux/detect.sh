...
flatpak list --columns=application 2>/dev/null | grep -qx "com.rafaelmardojai.Blanket" || exit 1
...
scope=--user
flatpak info --user com.rafaelmardojai.Blanket &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.rafaelmardojai.Blanket 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.rafaelmardojai.Blanket --show-commit 2>/dev/null)
