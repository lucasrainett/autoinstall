...
# `apt remove` silently drags out every reverse-dependency too. Uninstalling one catalog entry must
# never quietly take unrelated software with it, so refuse and report instead of guessing.
...
# Snapshots are deliberately left on disk: they are the user's backups, and removing the tool that
# manages them must not destroy them.
echo "Removed Timeshift. Any snapshots it took are still on disk."
