...
for scope in --user --system; do
  if flatpak info "$scope" app.zen_browser.zen &>/dev/null; then
    flatpak update "$scope" app.zen_browser.zen -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "app.zen_browser.zen is not installed in any flatpak installation." >&2
  exit 1
fi
