...
for scope in --user --system; do
  if flatpak info "$scope" com.valvesoftware.Steam &>/dev/null; then
    flatpak update "$scope" com.valvesoftware.Steam -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.valvesoftware.Steam is not installed in any flatpak installation." >&2
  exit 1
fi
