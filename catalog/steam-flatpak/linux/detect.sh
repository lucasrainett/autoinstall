...
flatpak list --columns=application 2>/dev/null | grep -qx "com.valvesoftware.Steam" || exit 1
...
scope=--user
flatpak info --user com.valvesoftware.Steam &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.valvesoftware.Steam 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.valvesoftware.Steam --show-commit 2>/dev/null)
