...
winget list --id OpenWhisperSystems.Signal -e --accept-source-agreements 2>/dev/null | grep -qi "OpenWhisperSystems.Signal" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id OpenWhisperSystems.Signal -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "OpenWhisperSystems.Signal"; then
  exit 2
fi
exit 0
