...
winget list --id cjpais.Handy -e --accept-source-agreements 2>/dev/null | grep -qi "cjpais.Handy" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id cjpais.Handy -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "cjpais.Handy"; then
  exit 2
fi
exit 0
