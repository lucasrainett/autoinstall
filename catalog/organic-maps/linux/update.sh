...
for scope in --user --system; do
  if flatpak info "$scope" app.organicmaps.desktop &>/dev/null; then
    flatpak update "$scope" app.organicmaps.desktop -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "app.organicmaps.desktop is not installed in any flatpak installation." >&2
  exit 1
fi
