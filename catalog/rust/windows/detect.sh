...
winget list --id Rustlang.Rustup -e --accept-source-agreements 2>/dev/null | grep -qi "Rustlang.Rustup" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Rustlang.Rustup -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Rustlang.Rustup"; then
  exit 2
fi
exit 0
