...
echo "Paint 3D is distributed through the Microsoft Store, which cannot be driven unattended."
echo "Opening the Store — search results for it will appear there."
powershell.exe -NoProfile -Command "Start-Process 'ms-windows-store://search/?query=Paint 3D'" 2>/dev/null || true
