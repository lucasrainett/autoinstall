...
flatpak list --columns=application 2>/dev/null | grep -qx "com.heroicgameslauncher.hgl" || exit 1
...
scope=--user
flatpak info --user com.heroicgameslauncher.hgl &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.heroicgameslauncher.hgl 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.heroicgameslauncher.hgl --show-commit 2>/dev/null)
