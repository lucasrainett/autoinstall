...
for scope in --user --system; do
  if flatpak info "$scope" com.heroicgameslauncher.hgl &>/dev/null; then
    flatpak update "$scope" com.heroicgameslauncher.hgl -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.heroicgameslauncher.hgl is not installed in any flatpak installation." >&2
  exit 1
fi
