...
for scope in --user --system; do
  if flatpak info "$scope" com.mojang.Minecraft &>/dev/null; then
    flatpak update "$scope" com.mojang.Minecraft -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.mojang.Minecraft is not installed in any flatpak installation." >&2
  exit 1
fi
