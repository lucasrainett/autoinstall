...
dpkg-query -W -f='${Status}' proton-pass 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' proton-pass 2>/dev/null)
candidate=$(apt-cache policy proton-pass 2>/dev/null | awk '/Candidate:/ {print $2}')
