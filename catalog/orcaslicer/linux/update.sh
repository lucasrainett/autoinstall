...
for scope in --user --system; do
  if flatpak info "$scope" com.orcaslicer.OrcaSlicer &>/dev/null; then
    flatpak update "$scope" com.orcaslicer.OrcaSlicer -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.orcaslicer.OrcaSlicer is not installed in any flatpak installation." >&2
  exit 1
fi
