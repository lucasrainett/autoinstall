...
dpkg-query -W -f='${Status}' python3-pip 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' python3-pip 2>/dev/null)
candidate=$(apt-cache policy python3-pip 2>/dev/null | awk '/Candidate:/ {print $2}')
