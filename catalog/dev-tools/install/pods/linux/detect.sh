...
flatpak list --columns=application 2>/dev/null | grep -qx "com.github.marhkb.Pods" || exit 1
...
scope=--user
flatpak info --user com.github.marhkb.Pods &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.github.marhkb.Pods 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.github.marhkb.Pods --show-commit 2>/dev/null)
