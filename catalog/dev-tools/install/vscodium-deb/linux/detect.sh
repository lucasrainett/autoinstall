...
dpkg-query -W -f='${Status}' codium 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' codium 2>/dev/null)
candidate=$(apt-cache policy codium 2>/dev/null | awk '/Candidate:/ {print $2}')
