...
flatpak list --columns=application 2>/dev/null | grep -qx "org.gnome.gitg" || exit 1
...
scope=--user
flatpak info --user org.gnome.gitg &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit org.gnome.gitg 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub org.gnome.gitg --show-commit 2>/dev/null)
