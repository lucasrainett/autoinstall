...
for scope in --user --system; do
  if flatpak info "$scope" io.github.flattool.Warehouse &>/dev/null; then
    flatpak update "$scope" io.github.flattool.Warehouse -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "io.github.flattool.Warehouse is not installed in any flatpak installation." >&2
  exit 1
fi
