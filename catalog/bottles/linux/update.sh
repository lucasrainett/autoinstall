...
for scope in --user --system; do
  if flatpak info "$scope" com.usebottles.bottles &>/dev/null; then
    flatpak update "$scope" com.usebottles.bottles -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.usebottles.bottles is not installed in any flatpak installation." >&2
  exit 1
fi
