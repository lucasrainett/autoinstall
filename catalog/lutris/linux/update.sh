...
for scope in --user --system; do
  if flatpak info "$scope" net.lutris.Lutris &>/dev/null; then
    flatpak update "$scope" net.lutris.Lutris -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "net.lutris.Lutris is not installed in any flatpak installation." >&2
  exit 1
fi
