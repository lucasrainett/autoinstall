...
winget list --id Anthropic.ClaudeCode -e --accept-source-agreements 2>/dev/null | grep -qi "Anthropic.ClaudeCode" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Anthropic.ClaudeCode -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Anthropic.ClaudeCode"; then
  exit 2
fi
exit 0
