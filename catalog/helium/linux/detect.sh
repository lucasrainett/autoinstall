...
flatpak run it.mijorus.gearlever --list-installed 2>/dev/null | grep -qi '^Helium' || exit 1

# Gear Lever tracks each AppImage's update source and reports the ones with updates available.
if flatpak run it.mijorus.gearlever --list-updates 2>/dev/null | grep -qi 'helium'; then
  exit 2
fi
exit 0
