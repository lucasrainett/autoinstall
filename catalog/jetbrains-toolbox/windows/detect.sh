...
winget list --id JetBrains.Toolbox -e --accept-source-agreements 2>/dev/null | grep -qi "JetBrains.Toolbox" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id JetBrains.Toolbox -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "JetBrains.Toolbox"; then
  exit 2
fi
exit 0
