...
flatpak list --columns=application 2>/dev/null | grep -qx "io.github.kolunmi.Bazaar" || exit 1
...
scope=--user
flatpak info --user io.github.kolunmi.Bazaar &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit io.github.kolunmi.Bazaar 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub io.github.kolunmi.Bazaar --show-commit 2>/dev/null)
