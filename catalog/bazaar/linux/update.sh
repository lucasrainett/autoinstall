...
for scope in --user --system; do
  if flatpak info "$scope" io.github.kolunmi.Bazaar &>/dev/null; then
    flatpak update "$scope" io.github.kolunmi.Bazaar -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "io.github.kolunmi.Bazaar is not installed in any flatpak installation." >&2
  exit 1
fi
