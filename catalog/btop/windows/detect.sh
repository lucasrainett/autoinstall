...
winget list --id aristocratos.btop4win -e --accept-source-agreements 2>/dev/null | grep -qi "aristocratos.btop4win" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id aristocratos.btop4win -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "aristocratos.btop4win"; then
  exit 2
fi
exit 0
