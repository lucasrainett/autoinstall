...
dpkg-query -W -f='${Status}' git 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' git 2>/dev/null)
candidate=$(apt-cache policy git 2>/dev/null | awk '/Candidate:/ {print $2}')
