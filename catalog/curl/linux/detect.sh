...
dpkg-query -W -f='${Status}' curl 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' curl 2>/dev/null)
candidate=$(apt-cache policy curl 2>/dev/null | awk '/Candidate:/ {print $2}')
