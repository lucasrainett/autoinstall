...
for scope in --user --system; do
  if flatpak info "$scope" com.moonlight_stream.Moonlight &>/dev/null; then
    flatpak update "$scope" com.moonlight_stream.Moonlight -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.moonlight_stream.Moonlight is not installed in any flatpak installation." >&2
  exit 1
fi
