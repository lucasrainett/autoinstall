...
winget list --id VSCodium.VSCodium -e --accept-source-agreements 2>/dev/null | grep -qi "VSCodium.VSCodium" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id VSCodium.VSCodium -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "VSCodium.VSCodium"; then
  exit 2
fi
exit 0
