...
for scope in --user --system; do
  if flatpak info "$scope" org.angryip.ipscan &>/dev/null; then
    flatpak update "$scope" org.angryip.ipscan -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.angryip.ipscan is not installed in any flatpak installation." >&2
  exit 1
fi
