...
for scope in --user --system; do
  if flatpak info "$scope" org.gimp.GIMP &>/dev/null; then
    flatpak update "$scope" org.gimp.GIMP -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.gimp.GIMP is not installed in any flatpak installation." >&2
  exit 1
fi
