...
winget list --id Guru3D.Afterburner -e --accept-source-agreements 2>/dev/null | grep -qi "Guru3D.Afterburner" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Guru3D.Afterburner -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Guru3D.Afterburner"; then
  exit 2
fi
exit 0
