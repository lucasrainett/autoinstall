...
for scope in --user --system; do
  if flatpak info "$scope" dev.lizardbyte.app.Sunshine &>/dev/null; then
    flatpak update "$scope" dev.lizardbyte.app.Sunshine -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "dev.lizardbyte.app.Sunshine is not installed in any flatpak installation." >&2
  exit 1
fi
