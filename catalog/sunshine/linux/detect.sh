...
flatpak list --columns=application 2>/dev/null | grep -qx "dev.lizardbyte.app.Sunshine" || exit 1
...
scope=--user
flatpak info --user dev.lizardbyte.app.Sunshine &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit dev.lizardbyte.app.Sunshine 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub dev.lizardbyte.app.Sunshine --show-commit 2>/dev/null)
