...
flatpak list --columns=application 2>/dev/null | grep -qx "org.videolan.VLC" || exit 1
...
scope=--user
flatpak info --user org.videolan.VLC &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit org.videolan.VLC 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub org.videolan.VLC --show-commit 2>/dev/null)
