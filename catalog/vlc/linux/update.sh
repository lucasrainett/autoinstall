...
for scope in --user --system; do
  if flatpak info "$scope" org.videolan.VLC &>/dev/null; then
    flatpak update "$scope" org.videolan.VLC -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.videolan.VLC is not installed in any flatpak installation." >&2
  exit 1
fi
