...
for scope in --user --system; do
  if flatpak info "$scope" ai.jan.Jan &>/dev/null; then
    flatpak update "$scope" ai.jan.Jan -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "ai.jan.Jan is not installed in any flatpak installation." >&2
  exit 1
fi
