...
for scope in --user --system; do
  if flatpak info "$scope" com.github.marhkb.Pods &>/dev/null; then
    flatpak update "$scope" com.github.marhkb.Pods -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.github.marhkb.Pods is not installed in any flatpak installation." >&2
  exit 1
fi
