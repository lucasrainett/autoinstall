...
dpkg-query -W -f='${Status}' proton-vpn-gnome-desktop 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' proton-vpn-gnome-desktop 2>/dev/null)
candidate=$(apt-cache policy proton-vpn-gnome-desktop 2>/dev/null | awk '/Candidate:/ {print $2}')
