...
# sed yielded "/home/u/AppImages/beeper.appimage          ", awk yields it clean.
#
...
# named beeper.appimage lists as "beeper", not "Beeper". The case-sensitive /^Beeper/ found
...
APPIMAGE=$(flatpak run it.mijorus.gearlever --list-installed 2>/dev/null |
  awk 'tolower($0) ~ /^beeper/ { print $NF }' | head -1)

if [ -z "$APPIMAGE" ]; then
  echo "Beeper is not integrated with Gear Lever; nothing to remove."
  exit 0
fi
