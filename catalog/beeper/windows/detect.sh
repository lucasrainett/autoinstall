...
winget list --id Beeper.Beeper -e --accept-source-agreements 2>/dev/null | grep -qi "Beeper.Beeper" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Beeper.Beeper -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Beeper.Beeper"; then
  exit 2
fi
exit 0
