...
flatpak list --columns=application 2>/dev/null | grep -qx "re.sonny.Junction" || exit 1
...
scope=--user
flatpak info --user re.sonny.Junction &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit re.sonny.Junction 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub re.sonny.Junction --show-commit 2>/dev/null)
