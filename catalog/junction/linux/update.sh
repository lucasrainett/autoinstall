...
for scope in --user --system; do
  if flatpak info "$scope" re.sonny.Junction &>/dev/null; then
    flatpak update "$scope" re.sonny.Junction -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "re.sonny.Junction is not installed in any flatpak installation." >&2
  exit 1
fi
