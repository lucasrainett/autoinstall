...
for scope in --user --system; do
  if flatpak info "$scope" org.signal.Signal &>/dev/null; then
    flatpak update "$scope" org.signal.Signal -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.signal.Signal is not installed in any flatpak installation." >&2
  exit 1
fi
