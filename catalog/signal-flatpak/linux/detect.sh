...
flatpak list --columns=application 2>/dev/null | grep -qx "org.signal.Signal" || exit 1
...
installed=$(flatpak info "$scope" --show-commit org.signal.Signal 2>/dev/null) || exit 0
