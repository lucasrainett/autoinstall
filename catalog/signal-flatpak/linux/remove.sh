...
if flatpak info --user org.signal.Signal &>/dev/null; then
  flatpak uninstall --user org.signal.Signal -y
elif flatpak info --system org.signal.Signal &>/dev/null; then
  flatpak uninstall --system org.signal.Signal -y
fi
