...
dpkg-query -W -f='${Status}' jq 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' jq 2>/dev/null)
candidate=$(apt-cache policy jq 2>/dev/null | awk '/Candidate:/ {print $2}')
