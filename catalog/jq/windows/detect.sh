...
winget list --id jqlang.jq -e --accept-source-agreements 2>/dev/null | grep -qi "jqlang.jq" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id jqlang.jq -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "jqlang.jq"; then
  exit 2
fi
exit 0
