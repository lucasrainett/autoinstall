...
dpkg-query -W -f='${Status}' proton-mail 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' proton-mail 2>/dev/null)
candidate=$(apt-cache policy proton-mail 2>/dev/null | awk '/Candidate:/ {print $2}')
