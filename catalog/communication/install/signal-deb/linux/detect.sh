...
dpkg-query -W -f='${Status}' signal-desktop 2>/dev/null | grep -q '^install ok installed$' || exit 1
...
installed=$(dpkg-query -W -f='${Version}' signal-desktop 2>/dev/null)
candidate=$(apt-cache policy signal-desktop 2>/dev/null | awk '/Candidate:/ {print $2}')
