...
winget list --id RedHat.Podman-Desktop -e --accept-source-agreements 2>/dev/null | grep -qi "RedHat.Podman-Desktop" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id RedHat.Podman-Desktop -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "RedHat.Podman-Desktop"; then
  exit 2
fi
exit 0
