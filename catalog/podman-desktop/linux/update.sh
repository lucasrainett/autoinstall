...
for scope in --user --system; do
  if flatpak info "$scope" io.podman_desktop.PodmanDesktop &>/dev/null; then
    flatpak update "$scope" io.podman_desktop.PodmanDesktop -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "io.podman_desktop.PodmanDesktop is not installed in any flatpak installation." >&2
  exit 1
fi
