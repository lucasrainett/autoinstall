...
for scope in --user --system; do
  if flatpak info "$scope" com.github.marktext.marktext &>/dev/null; then
    flatpak update "$scope" com.github.marktext.marktext -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.github.marktext.marktext is not installed in any flatpak installation." >&2
  exit 1
fi
