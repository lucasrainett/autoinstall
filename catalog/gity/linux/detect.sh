...
flatpak list --columns=application 2>/dev/null | grep -qx "com.markdeepwell.GitY" || exit 1
...
scope=--user
flatpak info --user com.markdeepwell.GitY &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit com.markdeepwell.GitY 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub com.markdeepwell.GitY --show-commit 2>/dev/null)
