...
for scope in --user --system; do
  if flatpak info "$scope" com.markdeepwell.GitY &>/dev/null; then
    flatpak update "$scope" com.markdeepwell.GitY -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.markdeepwell.GitY is not installed in any flatpak installation." >&2
  exit 1
fi
