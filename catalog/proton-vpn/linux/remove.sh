...
# Also remove Proton's "release" package, which is what registered their apt repo in the
# first place (install.sh installs it before the app itself).
sudo apt remove -y protonvpn-stable-release 2>/dev/null || true
