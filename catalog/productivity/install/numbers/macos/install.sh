...
if [ -d "/Applications/Numbers.app" ]; then
  echo "Numbers is already installed."
  exit 0
fi

echo "Numbers is distributed through the Mac App Store, which cannot be driven unattended."
echo "Opening its App Store page — press Get/Download there to reinstall it."
open "macappstore://apps.apple.com/app/id409203825" 2>/dev/null || true
