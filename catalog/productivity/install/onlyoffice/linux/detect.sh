...
flatpak list --columns=application 2>/dev/null | grep -qx "org.onlyoffice.desktopeditors" || exit 1
...
scope=--user
flatpak info --user org.onlyoffice.desktopeditors &>/dev/null || scope=--system
...
installed=$(flatpak info "$scope" --show-commit org.onlyoffice.desktopeditors 2>/dev/null) || exit 0
remote=$(flatpak remote-info "$scope" --cached flathub org.onlyoffice.desktopeditors --show-commit 2>/dev/null)
