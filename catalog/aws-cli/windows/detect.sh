...
winget list --id Amazon.AWSCLI -e --accept-source-agreements 2>/dev/null | grep -qi "Amazon.AWSCLI" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Amazon.AWSCLI -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Amazon.AWSCLI"; then
  exit 2
fi
exit 0
