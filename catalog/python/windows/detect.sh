...
winget list --id Python.Python.3 -e --accept-source-agreements 2>/dev/null | grep -qi "Python.Python.3" || exit 1

# `winget upgrade --id` lists the package only when an upgrade is actually available.
if winget upgrade --id Python.Python.3 -e --accept-source-agreements --disable-interactivity 2>/dev/null | grep -qi "Python.Python.3"; then
  exit 2
fi
exit 0
