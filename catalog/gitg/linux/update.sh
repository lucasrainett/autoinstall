...
for scope in --user --system; do
  if flatpak info "$scope" org.gnome.gitg &>/dev/null; then
    flatpak update "$scope" org.gnome.gitg -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "org.gnome.gitg is not installed in any flatpak installation." >&2
  exit 1
fi
