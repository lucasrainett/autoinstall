...
for scope in --user --system; do
  if flatpak info "$scope" com.github.tchx84.Flatseal &>/dev/null; then
    flatpak update "$scope" com.github.tchx84.Flatseal -y --noninteractive
    updated=1
  fi
done

if [ "$updated" -eq 0 ]; then
  echo "com.github.tchx84.Flatseal is not installed in any flatpak installation." >&2
  exit 1
fi
