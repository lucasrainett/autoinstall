...
const VLC = entry({ category: "multimedia", kind: "install", id: "vlc" });
const CATALOG = [SIGNAL, VLC];
const SNAPSHOT = [
  unsatisfied("communication/install/signal"),
  unsatisfied("multimedia/install/vlc"),
];
  ...
  Deno.test(`runCli - dry-run never calls the runner, selection size ${selectionSize}`, async () => {
    const selectedKeys = new Set(
      selectionSize === 0
        ...
        : ["communication/install/signal", "multimedia/install/vlc"],
    );
...
Deno.test("runCli - dry-run's plan and rendered text reflect the real selection", async () => {
  const result = await runCli("dry-run", {
    catalog: CATALOG,
    platform: "linux",
    selectedKeys: new Set(["communication/install/signal"]),
    snapshot: SNAPSHOT,
  });
  if (result.kind !== "dry-run") throw new Error("expected dry-run");
  assertEquals(result.plan.actions.map((a) => a.key), ["communication/install/signal"]);
...
Deno.test("runCli - unattended mode actually runs every action in the plan, in order", async () => {
  ...
  const result = await runCli("unattended", {
    catalog: CATALOG,
    platform: "linux",
    selectedKeys: new Set(["communication/install/signal", "multimedia/install/vlc"]),
    snapshot: SNAPSHOT,
    runAction: fn,
  });
  if (result.kind !== "unattended") throw new Error("expected unattended");
  assertEquals(calls, ["communication/install/signal", "multimedia/install/vlc"]);
...
Deno.test("runCli - unattended execution is identical to independently calling computePlan + executePlanActions with the same runner", async () => {
  ...
  const selectedKeys = new Set(["communication/install/signal", "multimedia/install/vlc"]);
  ...
  const behavior = (key: string): Promise<PlanActionResult> =>
    Promise.resolve(
      key === "multimedia/install/vlc" ? { ok: false, error: "network unreachable" } : { ok: true },
    );
