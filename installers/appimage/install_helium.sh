...
[ "$AUTOINSTALL_UPDATE" != "true" ] && ls ~/AppImages/*[Hh]elium* &>/dev/null && echo "Helium already installed, skipping." && exit 0

cd ~/Downloads
VERSION=$(curl -s https://api.github.com/repos/imputnet/helium-linux/releases/latest | grep -oP '"tag_name": "\K[^"]+')
wget -q --show-progress -O helium.AppImage "https://github.com/imputnet/helium-linux/releases/download/${VERSION}/helium-${VERSION}-x86_64.AppImage"
